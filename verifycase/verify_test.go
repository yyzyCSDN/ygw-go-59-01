...
func TestLeaseExpiryStopsIssuing(t *testing.T) {
	metrics := metric.New()
	...
	_, err = allocator.Issue(context.Background(), active.ID, info.ID, 3)
	if !errors.Is(err, model.ErrLeaseInvalid) {
		t.Fatalf("issue with expired lease must fail with ErrLeaseInvalid, got %v", err)
	}
}
